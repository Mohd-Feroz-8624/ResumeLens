...
const ROLE_KEYWORDS = {
  frontend: [
    'javascript', 'react', 'html', 'css', 'typescript', 'redux', 'webpack', 'vite', 
    'tailwind', 'ui/ux', 'rest api', 'next.js', 'vue', 'angular', 'sass', 'responsive'
  ],
  backend: [
    'node.js', 'express', 'python', 'django', 'sql', 'postgresql', 'mongodb', 'rest api', 
    'redis', 'docker', 'microservices', 'java', 'spring', 'go', 'graphql', 'mysql'
  ],
  data: [
    'python', 'sql', 'excel', 'tableau', 'powerbi', 'pandas', 'numpy', 'statistics', 
    'data visualization', 'r', 'machine learning', 'analytics', 'dashboard', 'database'
  ],
  devops: [
    'docker', 'kubernetes', 'aws', 'ci/cd', 'terraform', 'linux', 'git', 'jenkins', 
    'ansible', 'shell scripting', 'monitoring', 'azure', 'gcp', 'cloud'
  ],
  product: [
    'agile', 'scrum', 'roadmap', 'market research', 'user stories', 'analytics', 
    'jira', 'product design', 'strategy', 'metrics', 'backlog', 'stakeholder'
  ],
  general: [
    'communication', 'problem solving', 'teamwork', 'leadership', 'collaboration', 
    'analytical thinking', 'management', 'git', 'agile', 'project management'
  ]
};
...
function getCategoryForRole(role) {
  ...
  if (r.includes('back') || r.includes('api') || r.includes('server')) return 'backend';
  if (r.includes('data') || r.includes('analy') || r.includes('science')) return 'data';
  if (r.includes('devops') || r.includes('infra') || r.includes('cloud') || r.includes('system')) return 'devops';
  if (r.includes('product') || r.includes('project') || r.includes('manager')) return 'product';
  return 'general';
}
...
router.post('/', upload.single('resume'), async (req, res) => {
  try {
    ...
    const combinedKeywords = [...new Set([...technicalKeywords, ...generalKeywords])];
    ...
    combinedKeywords.forEach((kw) => {
      ...
      const hasMatch = regex.test(normalizedText) || normalizedText.includes(kw);
      
      const displayWord = kw.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
      if (hasMatch) {
    ...
    const matchRatio = combinedKeywords.length > 0 ? matched.length / combinedKeywords.length : 1;
    ...
    const SECTION_PATTERNS = {
      ...
      Skills: /\b(skills|technologies|proficiencies|expertise|technical skill)\b/i,
    ...
    if (missing.length > 0) {
      ...
      suggestions.push(`Include target industry keywords like: ${topMissing} to better align with ${role} requirements.`);
    }

    // Layout/Sections suggestions
    missingSections.forEach(sec => {
      suggestions.push(`Add a distinct "${sec}" section heading to structure your achievements clearly.`);
    });

    // Contact info suggestions
    missingContact.forEach(item => {
      suggestions.push(`Provide your ${item} in the contact header to help recruiters reach you.`);
    });

    // Experience/Writing optimization suggestions
    if (experience && parseInt(experience, 10) > 0) {
      suggestions.push('Quantify experience impact with concrete metrics (e.g., "led team of 4", "scaled throughput by 30%").');
    }
    suggestions.push('Use active verb phrases like: built, designed, optimized, shipped, and led to describe achievements.');
    suggestions.push('Keep formatting clean and ensure sections are easily identifiable for automated parsing.');
